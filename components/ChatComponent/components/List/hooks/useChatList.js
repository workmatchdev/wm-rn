...
const useChatList = () => {
    ...
    useEffect(() => {
        const getChats = async () => {
            try {
                if (!user) return null
                setLoading(true);
                const response = await axios.get(`https://workmatch-server-0c86658d19cb.herokuapp.com/api/chats/getChats/${user._id}`);
    ...
    const handleUpdateMessageStatus = async (chatId) => {
        try {
            const query = await axios.get(`https://workmatch-server-0c86658d19cb.herokuapp.com/api/chats/updateMessageStatus/${chatId}/${user._id}`)
            return query
        } catch (error) {
            console.log('error');
        }
